...
void RobotManager::onNewRobotConnection(websocket::stream<tcp::socket> robot_websocket) {
    ...
    try {
        ...
        robot_websocket.async_read(*buffer, [this, timer, buffer, response_message, &robot_websocket](beast::error_code ec, std::size_t bytes_transferred) {
            ...
            // Search for robot with the same MAC address in DB
            auto filter = bsoncxx::builder::stream::document{} << "mac_address" << mac_address << bsoncxx::builder::stream::finalize;
            auto result = DatabaseDAO::findDocument("Robots", filter.view());

            if (result) {
                std::cout << "INFO : Robot with MAC address " << mac_address << " already exists in the database" << std::endl;

                //READ THERE !!!
                bsoncxx::document::view view = result->view();
                for (const auto& element : view) {
                    std::cout << "Key: " << element.key().to_string() << ", ";

                    switch (element.type()) {
                        case bsoncxx::type::k_utf8:
                            std::cout << "Value: " << element.get_utf8().value.to_string() << std::endl;
                            break;
                        case bsoncxx::type::k_int32:
                            std::cout << "Value: " << element.get_int32() << std::endl;
                            break;
                        case bsoncxx::type::k_bool:
                            std::cout << "Value: " << std::boolalpha << element.get_bool() << std::endl;
                            break;
                        default:
                ...
                //auto db_id = view["_id"]["$oid"].get_utf8().value.to_string();
                auto db_id = view["_id"].get_oid().value.to_string();
                std::string db_name = view["name"].get_utf8().value.to_string();
                std::string db_mac_address = view["mac_address"].get_utf8().value.to_string();
                std::string db_ip = view["ip"].get_utf8().value.to_string();
                bool db_is_accepted = view["isAccepted"].get_bool().value;

                if(db_name != internal_robot_name || db_mac_address != mac_address) {
                    std::cout << "DEBUG : read : | db_name : " << db_name << " | internal_robot_name : " << internal_robot_name << std::endl;
                    std::cout << "DEBUG : read : | db_mac_address : " << db_mac_address << " | mac_address : " << mac_address << std::endl;
                    return;
                }

                // Create Robot object
                std::shared_ptr<Robot> robot = std::make_shared<Robot>(robot_websocket, _ioc, db_id, internal_robot_name, mac_address, ip, port, db_is_accepted, std::bind( &RobotManager::onRobotDisconnected, this, std::placeholders::_1 ) );

                //Put robot into _robots
                _robots.emplace(mac_address, robot);
            } else {
                std::cout << "INFO : Robot with MAC address " << mac_address << " doesn't exist in the database" << std::endl;

                // Create Robot object
                //Robot robot(robot_websocket, _ioc, "", internal_robot_name, mac_address, ip, port, false, [this](auto && PH1) { onRobotDisconnected(std::forward<decltype(PH1)>(PH1)); });
                std::shared_ptr<Robot> robot = std::make_shared<Robot>(robot_websocket, _ioc, "", internal_robot_name, mac_address, ip, port, false, std::bind( &RobotManager::onRobotDisconnected, this, std::placeholders::_1 ) );
...
void RobotManager::onRobotDisconnected(const std::string &mac_address) {
    const auto it = _robots.find(mac_address);
    if (it != _robots.end()) {
        _robots.erase(it);
    }
