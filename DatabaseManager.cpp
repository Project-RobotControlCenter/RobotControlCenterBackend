...
DatabaseManager::DatabaseManager(const char *local_db_ip, const char *local_db_port, const char *remote_db_ip,
    ...
    connect_to_local = connect(local_db_ip, local_db_port, db_password);
    if(connect_to_local) {
    ...
    connect_to_remote = connect(remote_db_ip, remote_db_port, db_password);
    ...
    if(connect_to_local || connect_to_remote) {
...
bool DatabaseManager::connect(const char *db_ip, const char *db_port, const char *db_password) {
    try {
        const std::string url = "mongodb://admin:" + std::string(db_password) + "@" + std::string(db_ip) + ":" + std::string(db_port);
        _client = std::make_unique<mongocxx::client>(mongocxx::uri{url});
